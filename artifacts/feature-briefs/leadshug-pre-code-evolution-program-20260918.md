...
| `ST-03` | Modelar conceitualmente canais, filas/equipes e formas de atendimento a partir das lições do whatsflow_v2. | inbox-and-conversations | integrations-and-channels, identity-and-tenancy, audit-and-history | Diagrama conceitual, cardinalidades, vocabulário, invariantes, estados, roteamento, autorização e anti-padrões são documentados; nenhuma decisão é silenciosamente canonizada. | Cenários de entrada, atribuição, capacidade, transferência, SLA, múltiplas BUs e isolamento entre Mantenedoras. | [TODO de planejamento ativo](../../todos/active/features/TODO-leadshug-whatsflow-channel-attendance-study.md) e [`BLG-whatsflow-channel-attendance-study`](../../backlog/README.md) | Validar as três decisões materiais do estudo; nenhuma execução ou canonização autorizada. | Deve separar inspiração útil de dívida observada. |
| `ST-04` | Sintetizar os estudos em mapa de capacidades, decisões e sequência de evolução do LeadsHug. | system roadmap | módulos e contracts | O roadmap passa a apontar para capacidades e decisões priorizadas, com dependências e critérios de saída; candidatos não aprovados permanecem fora de `todos/active/`. | Revisão de rastreabilidade: cada prioridade volta à evidência e avança para um TODO apenas quando aprovada. | [`BLG-leadshug-evolution-synthesis`](../../backlog/README.md) | ST-01 e ST-02 concluídos; depende agora da conclusão do ST-03. | É a convergência do programa, não uma implementação de produto. |
