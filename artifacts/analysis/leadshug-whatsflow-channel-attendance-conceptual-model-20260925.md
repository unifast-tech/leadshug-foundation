...
| E-23 | `whatsflow_v2@3a36436c83ebefc6839380eb8fac1a6f13f4700a:src/hooks/whatsapp/useConversations.ts:resolveConversation:724-772` | resolve faz `update whatsapp_leads` e, após sucesso, `insert whatsapp_messages` sequencial; o blob não mostra transação, CAS, nem predicado explícito de account no update |
...
| E-28 | `whatsflow_v2@3a36436c83ebefc6839380eb8fac1a6f13f4700a:supabase/functions/uazapi-webhook/index.ts:Deno.serve:1021-1127` | reabertura/assignment/`claimed_at` observados em writes sequenciais; select/update e predicates são tenant/instance ou id conforme trecho, sem prova de transação/CAS/account predicate completo |
...
| E-NF-01 | Protocolo reproduzível e fail-closed: `git -C '<snapshot>' grep -n -I -E 'round.?robin|least_busy|distribution_mode|agent_status|assigned_attendant_id|max_conversations|current_conversations|last_activity_at|heartbeat|last_seen|capacity|attendant.?presence|claim|scheduler|ttl|fresh' 3a36436c83ebefc6839380eb8fac1a6f13f4700a -- 'src/hooks/whatsapp/useConversations.ts' 'src/hooks/useSectorAccess.ts' 'src/hooks/useConversationSla.ts' 'src/components/whatsapp/WhatsAppLayout.tsx' 'src/components/whatsapp/panels/ChatPanel.tsx' 'src/components/whatsapp/panels/LeftPanel.tsx' 'src/components/whatsapp/left/NewConversationDialog.tsx' 'src/components/mensageria/sla/SlaBadge.tsx' 'src/components/mensageria/inbox/InboxTab.tsx' 'src/pages/MensageriaPage.tsx' 'src/components/mensageria/agents/AgentDashboard.tsx' 'src/components/mensageria/agents/DepartmentManager.tsx' 'supabase/functions/meta-webhook/index.ts' 'supabase/functions/uazapi-webhook/index.ts' 'supabase/functions/automation-router/index.ts' ':(glob)supabase/migrations/*.sql' '.context/05_database_schema.md' '.context/15_known_issues_and_debt.md'`; a allowlist cobre o corpus operacional admitido e exclui `docs/memorandos/**`, `docs/**` não nomeados e `_user_prompts_raw.json`; todo match deve ser classificado antes de afirmar ausência. | Matches observados: `agent_status`/contagens (E-15), `assigned_attendant_id` (E-13/E-27/E-28/E-29), `ultimo_ping` de provider (E-28), configuração de modos (E-16). Ausência limitada após protocolo: consumer/scheduler de distribuição, TTL/freshness de **atendente**, reserva/capacidade e CAS de claim. Provider-presence, status/configuração, webhooks/router não pertencem à alegação de ausência. |
...
| C-04 unidade/equipe | [E-04], [E-05], [E-22]; `direct`, `partial`. Claim UI/membership não requer schema; o write observado `whatsapp_instances.department_id` (E-04) tem schema estático, enquanto schema/ACL efetivos de `agent_departments` são `partial/unknown`. | não reutilizar Setor; unidade/equipe especializa atendimento e só se relaciona a BUs explicitamente. **Positivo:** agente com membership da unidade *e* grant da BU atende a BU vinculada. **Negativo:** membership isolado ou `department_id` legado coincidente não permite view/claim de uma BU sem grant. | `anti_pattern`: `department` legado = Setor. E-22 é fail-open de UI, não auth server-side. `future_decision`: entidade/ownership da unidade. |
| C-05 associação/cobertura | [E-06a], [E-06b], [E-22]; `direct`, `partial`. Gate/diálogo UI são `not_applicable` a schema; as leituras de profile/`agent_departments` de E-22 são persistência observada, porém schema/ACL efetivo `partial/unknown`. | `leadshug_constraint`: grants na BU; leitura independente. `legacy_observation`: filtro/membership UI não prova grant. | `limitation`: E-22 permite view-all sem departments, `department_id` nulo e atribuição direta no filtro UI; não demonstra grant. |
| C-06 fila/elegibilidade | [E-01], [E-09], [E-19], [E-20], [E-28], [E-22]; `direct`, `partial`. Abas/filtros UI são `not_applicable`; writes de lead/webhook são persistence-backed, com schema/escopo efetivo apenas `partial` e não prova de elegibilidade autorizada. | `leadshug_constraint`: fila scoped tenant/BU e fila livre não concede acesso. Webhook pode abrir/pendenciar lead; filtro UI não é eligibilidade autorizada. | `unknown`: predicado de elegibilidade/grant não demonstrado. |
| C-07 roteamento | [E-14], [E-16], [E-19], [E-20], [E-21], [E-29], [E-NF-01]; `direct/declarative_only`, `partial`. UI/configuração é `not_applicable`; `distribution_mode` é write persistente estático, e router/webhook writes são `partial`, sem scheduler/consumer efetivo. | `leadshug_constraint`: provider não escolhe agente. Cenário: modo/status nunca concede grant BU nem prova execução do algoritmo, freshness ou capacidade reservada. | `limitation`: configuração/fan-out/spread aleatório ≠ distribuição autorizada. `future_decision`: scheduler, critérios e fallback. |
| C-08 claim/atribuição/presença/capacidade | [E-01], [E-06a], [E-13], [E-15], [E-19], [E-20], [E-22], [E-27], [E-28], [E-29], [E-NF-01]; `partial`. UI/status são `not_applicable`; upserts/updates são persistência estática `partial`, sem schema/ACL runtime, CAS, reserva, TTL/freshness ou grant demonstrados. | `leadshug_constraint`: único owner e grants BU. Cenário: status/contagem/configuração nunca concede BU nem prova freshness/capacidade; dois claims exigem winner/loser. | `anti_pattern`: UI/polling/upsert como mutex. |
| C-09 transferência/handoff | [E-06b], [E-06c], [E-06d], [E-19], [E-20], [E-21], [E-28], [E-29]; `declarative_only`, `partial`. Diálogo UI é `not_applicable`; updates/upserts de lead são persistence-backed, mas schema/ACL/transação efetivos permanecem `partial/unknown`. | `leadshug_constraint`: origem/destino autorizados e uma posse. | `limitation`: updates/notes sequenciais não são transação/CAS. |
| C-10 lifecycle | [E-01], [E-02], [E-07], [E-09], [E-23], [E-28]; `conflicting`, `partial`. E-23/E-28 observam resolve/reopen e writes persistentes, mas não transação, CAS ou predicate account completo; E-07 é superseded. | aberto, pendente, em atendimento, resolvido, reaberto; ownership explícito. Entrada após resolve reabre sem apagar histórico. | `anti_pattern`: fundir status de UI/SLA/ownership. `future_decision`: transições autorizadas. |
...
| C-12 auditoria/histórico | [E-06c], [E-06d], [E-10], [E-13], [E-23], [E-28]; `direct`, `partial`; writes/inserts posteriores são persistência estática/sequencial, não prova de log append-only atômico/em runtime, nem predicate account completo. | append-oriented, tenant/BU scoped, leitura independente. Cross-tenant/cross-BU e write-sem-read são negativos obrigatórios. | `unknown`: evento operacional completo. `recommendation`: eventos imutáveis por transição. |

### Aplicabilidade de schema e matriz ator × operação

`not_applicable` vale somente para uma alegação sem persistência (por exemplo: aba/diálogo/gate de UI, status exibido, configuração declarativa). Onde a mesma linha contém write ou leitura persistente, schema aplicável é registrado como `effective_schema` quando E-17/E-02 o resolve, ou `partial/unknown` quando o blob só mostra `whatsapp_leads`, `agent_departments`, `departments` ou `conversations` sem DDL/ACL final. Assim C-04..C-09/C-12 não recebem `not_applicable` global: a interface não prova schema; cada write é avaliado separadamente. Nenhuma classificação estática prova runtime, autorização, atomicidade ou freshness.
...
| OWNER/ADMIN/ATENDENTE × claim | grant BU e ownership único | testar cada papel; mapeamento papel→claim e winner/loser são `future_decision/unknown` | sem escopo/grant não satisfaz a constraint; corrida exige CAS futuro | E-13/E-27/E-29 são upserts estáticos, sem CAS |
| OWNER/ADMIN/ATENDENTE × assign | tenant/account, grant BU e regra de operação explícita | testar cada papel; nenhum allow/deny por papel é inferido | tenant/account/grant/null-scope devem ser avaliados separadamente | E-06c/E-19/E-28 observam write, não autorização |
