...
The strategic steward opens a follow-up tactical TODO for a permanent validator when either condition occurs: more than ten live records combined across backlog, decisions, and roadmap, or the first proven recurrence of schema/field-authority drift. ST-01 reached the volume threshold with 18 live records. The delivered control is [`deterministic/validate_foundation_lifecycle.py`](deterministic/validate_foundation_lifecycle.py); its bounded source graph, read-only behavior, and exit contract are defined in [`deterministic/README.md`](deterministic/README.md). CI adoption remains a separately governed decision.
