...
- **Qualifiers:** `none`
- **Next exact step:** fechar `R14-CRIT2-01` refinando `D-38/D-45` com replacement-object disabled, publicar e executar R-15 até nenhum reviewer descobrir decisão approval-material nova.

## Active Work State

- **Work state:** `review`
- **Why this state now:** D-53/D-54 foram refinados/publicados em `11e6c5af`; R-14 confirmou sua coerência, mas encontrou `refs/replace` como input Git não fechado em D-38/D-45. A correção e R-15 precedem qualquer validação; implementação continua proibida.
...
- `deterministic/README.md` owns the immutable exact `Registry-v1` argv profile and its command-contract digest. The validator permanently exposes lazy compatibility/forensics modes `--run-attested-suite --tree <oid> --materialized-root <path>` and `--verify-attestation --commit <sha>` for that approved version. Registry-v1 generation remains executable as a historical compatibility profile, but its success asserts only Registry-v1 self-attestation and cannot satisfy a later delivery/current-governance gate unless a new approved TODO explicitly adopts it; any new current generation profile requires a new version and decision. Normal steady-state validation uses only `--root`, never resolves the sibling Delphi checkout and never runs legacy/governance commands.
...
- Every child environment is constructed from empty, never inherited. Its complete allowlist is `PATH={TOOL_DIR}`, `HOME={EMPTY_HOME}`, `XDG_CONFIG_HOME={EMPTY_XDG}`, `TMPDIR={RUN_TMP}`, `PYTHONDONTWRITEBYTECODE=1`, `PYTHONNOUSERSITE=1`, `LC_ALL=C.UTF-8`, `LANG=C.UTF-8`, `TZ=UTC`, `TERM=dumb`, `GIT_CONFIG_NOSYSTEM=1`, `GIT_CONFIG_GLOBAL=/dev/null`, `GIT_NO_REPLACE_OBJECTS=1`, `GIT_TERMINAL_PROMPT=0`, `GIT_PAGER=cat` and `PAGER=cat`; no other variable is passed. `{EMPTY_HOME}`/`{EMPTY_XDG}` are empty read-only directories and `{RUN_TMP}` is the only writable scratch directory, all outside admitted repositories. All direct Python argv place `-E -s -S -B` immediately after `{PYTHON}`; their only non-stdlib import roots are the attested script directory/cwd. Canonical sorted `NAME=value\0` template bytes are part of `registry-sha256` and separately bound by `environment-sha256`; physical capsule paths are represented only by their literal tokens.
...
- Git user/system configuration and ambient repository override variables are absent by construction. `GIT_NO_REPLACE_OBJECTS=1` is mandatory and registry/environment-bound for every Git subprocess and Git-using child, so captured commit/tree/blob reads, archive/materialization and post-commit verification never honor repository-local `refs/replace`. Local repository configuration remains required Git metadata and is bound through `git-config-manifest-sha256`, calculated over canonical path/type/bytes records for each file returned by `git rev-parse --git-path config` and the optional `config.worktree` in Foundation and Delphi. Before execution, bound Git runs with `--includes=false` to detect case-insensitively any `include.path` or `includeIf.*.path`; any unconditional/conditional include is rejected rather than resolved. Any config drift between capture and post-command verification fails.
...
- The pre-commit `--run-attested-suite` phase has one `30m` monotonic deadline covering capsule setup, five probes, all registry commands, materialization, identity/config checks, attestation generation and cleanup, reserving the final `30s` for cleanup. It stops launching work when remaining budget cannot cover the next fixed bound plus reserve; deadline or blocking setup/cleanup yields deterministic `suite-timeout|cleanup-timeout`, no attestation and nonzero exit. External commit creation is deliberately outside this process-local clock and has no attestation timing claim. The separate post-commit verifier has its own `5m` monotonic deadline, `1MiB` caps per stream and final `30s` cleanup reserve; it yields `verify-timeout|cleanup-timeout`, nonzero and no acceptance evidence on breach. No continuous cross-process wall-clock bound is claimed.
...
- Bound that verifier invocation and all of its setup/materialization/identity checks to the independent `5m` monotonic phase deadline, `1MiB` stdout/stderr caps and final `30s` cleanup reserve. A restarted verifier receives a fresh verifier-phase budget but can never reset or extend the already completed pre-commit phase; neither phase claims to bound the external commit interval.
- The verifier independently reads the exact commit message/tree through bound Git metadata, validates the complete envelope/payload/registry/tool/environment/config/trailer bindings and validates stream fields only for canonical inclusion and internal digest consistency; it cannot prove execution truth or recompute raw-stream digests. It never imports principal-worktree Python bytes. A principal-worktree mutation after commit must be rejected before execution; post-commit verification is publication-gate evidence and cannot be written back into the immutable commit.
...
| `D-38` | Construct every child environment from a complete explicit allowlist, use the verified private tool directory for all descendant lookups (including Delphi guards), isolate HOME/XDG/TMP/Git controls, mandate registry-bound `GIT_NO_REPLACE_OBJECTS=1`, bind local Git config manifests, and reject any ambient variable, unknown tool, PATH/config/capsule drift before acceptance. | `R7-ARCH-01`, `R7-CRIT1-01`, `R7-CRIT2-01`, `R14-CRIT2-01` | confirm descendant executable, inherited-environment and replacement-object closure |
...
| `D-45` | After commit, materialize and execute verifier code only from the captured HEAD tree under the same closed runner controls and replacement-disabled Git object reads; pass verified principal root and full commit OID as dedicated tokens, reject worktree ambiguity before execution and keep the result external to the immutable commit. | `R10-CRIT1-01`, `R14-CRIT2-01` | confirm final acceptance uses the named committed bytes without replacement substitution |
...
- [x] `D-38` Build all child environments from a complete allowlist, disable Git replacement objects and close descendant tool/Git configuration identity across every command context.
...
- [x] `D-45` Execute post-commit attestation verification exclusively from replacement-disabled Git reads and a read-only materialization of the captured HEAD tree.
...
- **Decision review evidence / resolution:** R-14 fresh architecture review over package SHA-256 `1d24eb16ab368256899ad5fea35a9f4e592f079bb64ef1f7bcb7f0e96acce1c5` at published head `11e6c5af`, 2026-09-24, was clean. Critique `R14-CRIT2-01` separately refined Git object-read closure, so fresh R-15 convergence and post-freeze rerun remain required.
...
| `R13-ARCH-01` | Integrated | useful | yes | project | `cross-process-deadline-boundary` | D-53 now uses separate process-local pre/post-commit deadlines and makes no continuous external-commit timing claim; pending reconvergence/revalidation |
...
- **Why this decision:** TODO high de architecture enforcement requer pacote estável/publicado antes da crítica.
...
| `R-15` | `origin/main at dispatch; exact commit + package SHA captured before review` | pending | pending | pending | no outcome yet | convergence round required |
...
| `T-34` | every command gets the complete from-empty environment, verified descendant tool directory, `GIT_NO_REPLACE_OBJECTS=1` and bound Git config manifest | unexpected inherited variable, `BASH_ENV`/`ENV`, Git override/config injection, missing/altered replacement disable, HOME/XDG influence, alternate index/worktree/object dir, PATH shadow in Delphi guard, capsule/config mutation | fail before/during execution; exact allowlist and pre/post tool/config/capsule identities; scratch writes only under `{RUN_TMP}` |
...
| `T-41` | post-commit verifier executes entrypoint/package from replacement-disabled Git reads and read-only captured HEAD materialization | mutate principal-worktree verifier/package after commit while HEAD remains unchanged; create commit/tree/blob `refs/replace`; remove/alter replacement disable; HEAD/tree/message mismatch | replacement refs never alter read/materialized bytes; mutable bytes are never executed; divergence fails before verifier; committed verifier accepts only exact envelope/bindings within D-48/D-52 trust limits |
...
- **Issue ID:** `PLAN-47` — the frozen Python probe contradicted the universal isolated-Python argv rule (`high`). Option A: add `-E -s -S -B` to the probe (recommended); Option B: create a probe exception. **Resolution:** integrated into `D-47`, probe grammar and `T-42`.
...
- **Issue ID:** `PLAN-53` — process groups were not adversarial containment and bounds cannot span external commit creation (`high`). Option A: best-effort supervision plus separate 30m pre-commit and 5m post-commit clocks (recommended); Option B: one new trusted coordinator/cgroup boundary. **Resolution:** integrated into `D-53` and `T-47`.
...
- **Needed:** `R-15 exploratory reconvergence after R-14 Git replacement-object closure; formal architecture/critique reruns remain required after replacement validation and freeze`
- **Why ambiguity remains:** `refined D-38/D-45 must survive one clean three-reviewer round before another human validation request`.
- **Opinion count:** `R-01 through R-14 complete; R-15 pending`
...
- **Findings summary:** R-14 architecture was clean and critique A had only bookkeeping; critique B found `R14-CRIT2-01`, a material Git replacement-object input, plus duplicate bookkeeping `R14-CRIT2-02`. The material finding refines `D-38/D-45`, the closed environment and `T-34/T-41`; fresh R-15 convergence and post-freeze critique remain required.
...
| `R12-CRIT2-05` | Integrated | useful | yes | project | `attested-suite-total-deadline` | refined after R-13 into enforceable 30-minute pre-commit and 5-minute post-commit phase deadlines with cleanup reserves; pending reconvergence/revalidation |
| `R13-CRIT1-01/R13-CRIT2-02` | Integrated | useful | yes | project | `attestation-registry-execution-lifecycle` | v1 remains runnable/verifiable only as historical-profile self-attestation and cannot become future-current evidence without explicit approved adoption; pending reconvergence/revalidation |
| `R13-CRIT1-02` | Integrated | useful | yes | paced | n/a | published-head/convergence/closeout bookkeeping reconciled before R-14 |
| `R13-CRIT2-01` | Integrated | useful | yes | project | `attested-suite-cross-phase-deadline` | pre-commit and post-commit processes now have explicit independent clocks/caps; external commit interval is outside the claim; pending reconvergence/revalidation |
| `R14-CRIT1-01/R14-CRIT2-02` | Integrated | useful | yes | paced | `review-package-current-state-coherence` | lifecycle/status fields reconciled to published R-14 outcome and R-15 next step |
...
- **Evidence / reference:** R-14 critiques over package SHA-256 `1d24eb16ab368256899ad5fea35a9f4e592f079bb64ef1f7bcb7f0e96acce1c5` at published head `11e6c5af`, 2026-09-24: critique A no approval-material blocker; critique B `material_findings_present; approval_not_ready` due to `R14-CRIT2-01`.
- **Waiver authority / reference:** `n/a`
...
| `R13-ARCH-01/R13-CRIT2-01` | high | release-blocker | refine phase deadline contract | a process-local monotonic clock cannot span externally coordinated commit creation | fixed-pending-convergence | refined `D-53/T-47` with 30m pre-commit and 5m post-commit bounds; full-set revalidation required |
| `R13-CRIT1-01/R13-CRIT2-02` | high | release-blocker | refine Registry-v1 applicability | permanent decoding/rerun support must not imply authority for future-current delivery evidence | fixed-pending-convergence | refined `D-54/T-48` historical-profile classification and explicit re-adoption rule; full-set revalidation required |
| `R13-CRIT1-02` | medium | release-blocker | reconcile bookkeeping | published package identity must agree with active state and convergence ledger | resolved | R-13 snapshot and next-step/closeout fields reconciled before R-14 |
| `R14-CRIT2-01` | high | release-blocker | refine Git execution closure | replacement refs are controllable repository inputs that can alter object reads despite stable OID text | fixed-pending-convergence | refined `D-38/D-45`, environment, `T-34/T-41`; full-set revalidation required |
...
- **Why this level:** não há endpoint/runtime de produto, mas o runner local supervisiona subprocessos, streams, descendants and cleanup; `D-40` limita recursos por comando, e testes adversariais devem provar esses limites.
...
- **Post-commit/push status:** invalidated freeze `3f351daf` and snapshots through R-14 head `11e6c5af` remain published provenance; replacement-object refinements must be verified on `origin/main` immediately before R-15 dispatch.
- **Next path/status action:** verify refined D-01..D-54 on `origin/main`, run R-15+ until a clean round, then request one renewed full-set validation before replacement freeze and formal gates.
